...
public class ResizeHeaderWithoutIndexTest extends AbstractResizeHeaderTest {
	...
	protected void resizeRowheader(final DataLayer tableRowIndexHeaderLayer, final DataLayer tableRowLabelHeaderLayer) {
		// Resize the row header
		final NatTable natTable = (NatTable) currentManager.getAdapter(NatTable.class);
		natTable.doCommand(new RowResizeCommand(tableRowLabelHeaderLayer, 0, 75));
		natTable.doCommand(new RowResizeCommand(tableRowLabelHeaderLayer, 1, 100));
		natTable.doCommand(new RowResizeCommand(tableRowLabelHeaderLayer, 2, 10));
		natTable.doCommand(new RowResizeCommand(tableRowLabelHeaderLayer, 3, 35));
		natTable.doCommand(new RowResizeCommand(tableRowLabelHeaderLayer, 4, 75));
		natTable.doCommand(new RowResizeCommand(tableRowLabelHeaderLayer, 5, 90));
	}
	...
	protected void checkModifiedRowHeaderWidth(final DataLayer tableRowIndexHeaderLayer, final DataLayer tableRowLabelHeaderLayer, final AbstractHeaderAxisConfiguration rowHeader) {
		Assert.assertEquals("The width of the label is modified", 75, tableRowIndexHeaderLayer.getRowHeightByPosition(0)); //$NON-NLS-1$
		Assert.assertEquals("The width of the label is modified", 100, tableRowLabelHeaderLayer.getRowHeightByPosition(1)); //$NON-NLS-1$
		Assert.assertEquals("The width of the label is modified", 10, tableRowLabelHeaderLayer.getRowHeightByPosition(2)); //$NON-NLS-1$
		Assert.assertEquals("The width of the label is modified", 35, tableRowLabelHeaderLayer.getRowHeightByPosition(3)); //$NON-NLS-1$
		Assert.assertEquals("The width of the label is modified", 75, tableRowLabelHeaderLayer.getRowHeightByPosition(4)); //$NON-NLS-1$
		Assert.assertEquals("The width of the label is modified", 90, tableRowLabelHeaderLayer.getRowHeightByPosition(5)); //$NON-NLS-1$
